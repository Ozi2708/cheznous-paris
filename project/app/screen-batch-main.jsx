/* ── Batch cooking — « Cuisine 2h le dimanche, mange toute la semaine. »
   1 · Idées de session (menus pré-composés) ou composition libre
   2 · Plan en un coup d'œil (phases repliables + matériel)
   3 · La semaine au frigo (frise de consommation)
   + Mode session 2h chrono multi-recettes. */
const { useState: useStateB, useMemo: useMemoB, useEffect: useEffectB } = React;

/* Couleurs attribuées aux plats d'une session (palette du système) */
const CN_BATCH_COLORS = ['#506741', '#8A6B4A', '#2E8B85', '#5C6FAE', '#E07534'];

/* ── Données dérivées : conservation & compatibilité ── */
function cnConservation(r) {
...
function cnBatchList() {
  /* compatibles : se conservent au moins 3 jours au frigo */
  return window.CHEZNOUS_DATA.recipes
...
/* ── Plan de session : étapes entrelacées ──
   1. On trie les plats par temps de cuisson décroissant.
   2. Pour chaque plat, on enchaîne ses étapes jusqu'à la première étape
      « passive » (four / mijotage) incluse → on passe au plat suivant.
   3. On termine en alternant les étapes restantes. */
function cnStepType(html) {
...
function cnBuildPlan(recipes) {
  ...
  /* À chaque tour, on suit un plat jusqu'à sa prochaine étape passive
     (four / mijotage) incluse, puis on bascule au plat suivant. */
  const plan = [];
  let moved = true;
  while (moved) {
    moved = false;
...
function cnSessionMinutes(recipes) {
  if (!recipes.length) return 0;
  return Math.max(...recipes.map(r => r.cookMin || 0)) + recipes.reduce((s, r) => s + (r.prepMin || 0), 0) + 5 * (recipes.length - 1);
}
function cnFmtDuration(min) {
  const h = Math.floor(min / 60), m = min % 60;
  return h > 0 ? `${h}h${m > 0 ? String(m).padStart(2, '0') : ''}` : `${m} min`;
}

/* ── Menus pré-composés (calculés sur les compatibles) ── */
function cnMenuIdeas(list) {
  const variee = [];
...
/* ── Matériel nécessaire (détecté dans les étapes) ── */
const CN_EQUIPMENT = [
...
function CNKeepBadges({ r, light }) {
  ...
  return (
    <span style={{ display: 'inline-flex', gap: 5 }}>
      ...
      {c.congel && <span style={{ ...base, background: light ? 'rgba(250,250,248,.14)' : '#EEF1FB', color: light ? '#FAFAF8' : '#5C6FAE' }}><CNIcon name="snow" size={11} color={light ? '#FAFAF8' : '#5C6FAE'}></CNIcon> congél.</span>}
    </span>
  );
}

/* ── Consolidation des ingrédients entre plats ── */
function cnIngKey(name) {
  return (name || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/\(s\)/g, '').trim().replace(/s$/, '');
}
const CN_STAPLES = ['sel', 'poivre'];
function cnConsolidate(selected) {
  const map = {};
...
/* ── Pourquoi ces plats ensemble ? (synergies calculées) ── */
function cnSynergies(selected) {
...
const CN_B_DAYS = ['Dim', 'Lun', 'Mar', 'Mer', 'Jeu', 'Ven', 'Sam'];
const CN_B_DAYS_FULL = ['dimanche', 'lundi', 'mardi', 'mercredi', 'jeudi', 'vendredi', 'samedi'];

/* ── Écran principal Batch ── */
function CNBatchScreen({ sel, setSel, onOpen, onStart }) {
  const list = useMemoB(() => cnBatchList(), []);
  const menus = useMemoB(() => cnMenuIdeas(list), [list]);
  ...
  const [previewId, setPreviewId] = useStateB(menus.length ? menus[0].id : null);
  const selected = sel.map(id => byId[id]).filter(Boolean);
  const minutes = cnSessionMinutes(selected);
  const { plan, colors } = useMemoB(() => selected.length >= 2 ? cnBuildPlan(selected) : { plan: [], colors: {} }, [sel.join()]);
  ...
  /* groupes consécutifs du plan → phases de la timeline */
  ...
  const equipment = useMemoB(() => selected.length >= 2 ? cnEquipment(selected) : [], [sel.join()]);
  const synergies = useMemoB(() => selected.length >= 2 ? cnSynergies(selected) : [], [sel.join()]);
  const consolidated = useMemoB(() => selected.length >= 2 ? cnConsolidate(selected) : [], [sel.join()]);
  const [soloOpen, setSoloOpen] = useStateB(false);
      ...
      <div style={{ flex: 1, overflowY: 'auto', minHeight: 0, paddingTop: 62 }}>
        ...
        <div style={{ padding: '10px 20px 18px', background: '#F9F1E7', borderBottom: '1.5px solid #EFE3D0' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
            <CNIcon name="pot" size={18} color="#8A6B4A"></CNIcon>
          ...
          <div style={{ fontFamily: CN_FONTS.serif, fontSize: 27, lineHeight: 1.18, color: '#1A1918' }}>
            Cuisine 2h le dimanche,<br></br><span style={{ fontStyle: 'italic', color: '#8A6B4A' }}>mange toute la semaine.</span>
          </div>
        </div>

        <div style={{ padding: '0 20px 170px' }}>
          {/* 1 · Idées de session : pastilles + carte d'aperçu */}
            ...
            {menus.map(menu => {
              ...
              return (
                <button key={menu.id} onClick={() => setPreviewId(menu.id)} style={{
                  ...
                  <CNIcon name={menu.icon} size={14} color={isPreview ? '#FAE9D7' : '#8A6B4A'}></CNIcon>
                  ...
                  {isSel && <CNIcon name="check" size={12} color={isPreview ? '#FAE9D7' : '#8A6B4A'} strokeWidth={2.5}></CNIcon>}
                </button>
              );
            })}
          </div>
              ...
              return (
                <div key={r.id} style={{
                  ...
                  <button onClick={() => toggle(r.id)} aria-label={on ? 'Retirer du batch' : 'Ajouter au batch'} style={{
                    ...
                    {on && <CNIcon name="check" size={15} color="#FFFFFF" strokeWidth={2.5}></CNIcon>}
                  </button>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 12 }}>
                  {selected.map(r => (
                    <button key={r.id} onClick={() => toggle(r.id)} title="Retirer" style={{
                      ...
                      {short(r.title, 24)}
                      <CNIcon name="x" size={10} color="#B8B3AA"></CNIcon>
                    </button>
                  ))}
                </div>
                      ...
                      {synergies.map((s, si) => (
                        <div key={si} style={{ display: 'flex', gap: 9, alignItems: 'flex-start' }}>
                          <span style={{ width: 22, height: 22, borderRadius: '50%', background: '#EDF1E7', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                            <CNIcon name={s.icon} size={12} color="#506741"></CNIcon>
                          </span>
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 5, flexWrap: 'wrap' }}>
                                ...
                                {g.cook && (
                                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontFamily: CN_FONTS.mono, fontSize: 10.5, color: g.oven ? '#E07534' : '#2E8B85', background: g.oven ? '#FDF0E7' : '#E7F1F0', borderRadius: 9999, padding: '2px 9px', whiteSpace: 'nowrap' }}>
                                    <CNIcon name={g.oven ? 'flame' : 'clock'} size={11} color={g.oven ? '#E07534' : '#2E8B85'}></CNIcon>
                                    puis {g.cook} min {g.oven ? 'au four' : 'de cuisson'}
                                  </span>
                                )}
                              </div>
                            </div>
                            <CNIcon name="chevR" size={15} color="#B8B3AA" style={{ transform: isOpen ? 'rotate(90deg)' : 'none', transition: 'transform .2s ease', flexShrink: 0 }}></CNIcon>
                          </div>
                          {isOpen && (
                      ...
                      {solo.length > 0 && (
                        <div>
                          <button onClick={() => setSoloOpen(!soloOpen)} style={{
                            ...
                            <CNIcon name="chevR" size={12} color="#B8B3AA" style={{ transform: soloOpen ? 'rotate(90deg)' : 'none', transition: 'transform .2s ease' }}></CNIcon>
      ...
      {selected.length > 0 && (
        <div style={{ position: 'absolute', left: 0, right: 0, bottom: 86, padding: '14px 20px 10px', background: 'linear-gradient(to top, #FAFAF8 70%, rgba(250,250,248,0))' }}>
          <button onClick={onStart} disabled={selected.length < 2} style={{
            ...
            <CNIcon name="pot" size={19} color="#FFFFFF"></CNIcon>
...
Object.assign(window, { CNBatchScreen, CNKeepBadges, cnConservation, cnBuildPlan, cnBatchList, cnStepType, cnSessionMinutes, cnFmtDuration, cnConsolidate, cnIngKey, cnSynergies, CN_BATCH_COLORS });
