/* ── Ma semaine — planning par jour, créneaux midi / soir ──────── */
const { useState: useStateW, useMemo: useMemoW } = React;

const CN_DAYS = ['Lundi', 'Mardi', 'Mercredi', 'Jeudi', 'Vendredi', 'Samedi', 'Dimanche'];
const CN_SLOTS = [{ id: 'midi', label: 'Midi' }, { id: 'soir', label: 'Soir' }];

/* Premier créneau libre à partir d'aujourd'hui (pour l'ajout rapide) */
function cnNextFreeSlot(week) {
...
/* Sheet de sélection d'une recette pour un créneau */
function CNPickerSheet({ open, onClose, onPick, slotLabel }) {
  const [q, setQ] = useStateW('');
  const all = window.CHEZNOUS_DATA.recipes;
  const results = useMemoW(() => q ? cnApplyFilters(all, { ...CN_EMPTY_FILTERS, q }) : all, [all, q]);
  return (
      ...
      <div style={{
        ...
        <div style={{ padding: '14px 22px 10px', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#FFFFFF', border: '1.5px solid #E4DDD2', borderRadius: 9999, padding: '0 16px', height: 42 }}>
            <CNIcon name="search" size={16} color="#B8B3AA"></CNIcon>
...
function CNWeekScreen({ week, setWeek, onOpen }) {
  const [picking, setPicking] = useStateW(null); /* {day, slot} */
  const [cleaning, setCleaning] = useStateW(false);
  const all = window.CHEZNOUS_DATA.recipes;
  const byId = useMemoW(() => Object.fromEntries(all.map(r => [r.id, r])), [all]);
  ...
  return (
    <div data-screen-label="Ma semaine" style={{ height: '100%', display: 'flex', flexDirection: 'column', background: '#FAFAF8', position: 'relative' }}>
                ...
                {CN_SLOTS.map(slot => {
                  ...
                  if (!r) return (
                    <button key={slot.id} onClick={() => setPicking({ day, slot: slot.id })} style={{
                      ...
                      <CNIcon name="plus" size={14} color="#B8B3AA"></CNIcon>
                  ...
                  return (
                    <div key={slot.id} style={{
                      ...
                      <button onClick={() => setSlot(day, slot.id, { ...entry, done: !done })} aria-label={done ? 'Marquer non cuisiné' : 'Marquer cuisiné'} style={{
                        ...
                        {done && <CNIcon name="check" size={15} color="#FFFFFF" strokeWidth={2.5}></CNIcon>}
                      </button>
                      <button onClick={() => setSlot(day, slot.id, null)} aria-label="Retirer" style={{
                        ...
                        <CNIcon name="x" size={14} color="#B8B3AA"></CNIcon>
                      </button>
                    </div>
                  );
                })}
      ...
      <CNPickerSheet open={!!picking} onClose={() => setPicking(null)}
        ...
        onPick={(r) => { if (picking) { setWeek({ ...week, [picking.day + '-' + picking.slot]: { id: r.id, done: false } }); setPicking(null); } }}></CNPickerSheet>
    </div>
  );
}

Object.assign(window, { CNWeekScreen, CN_DAYS, cnNextFreeSlot });
