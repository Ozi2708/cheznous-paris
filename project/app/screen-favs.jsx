/* ── Favoris — les recettes qu'on préfère ──────────────────────── */
const { useMemo: useMemoF } = React;

function CNFavsScreen({ favs, onToggleFav, onOpen, onQuickAdd }) {
  const all = window.CHEZNOUS_DATA.recipes;
  const list = useMemoF(() => favs.map(id => all.find(r => r.id === id)).filter(Boolean), [favs.join()]);
        ...
        {list.length === 0 && (
          <div style={{ textAlign: 'center', padding: '70px 26px 0' }}>
            <span style={{ display: 'inline-flex', width: 64, height: 64, borderRadius: '50%', background: '#F5F0E8', alignItems: 'center', justifyContent: 'center', marginBottom: 16 }}>
              <CNIcon name="heart" size={30} color="#B89268"></CNIcon>
            </span>
            ...
            return (
              <div key={r.id} style={{
                ...
                <button onClick={() => onToggleFav(r.id)} aria-label="Retirer des favoris" title="Retirer des favoris" style={{
                  ...
                  <CNIcon name="heart" size={19} color="#B89268" fill="#B89268"></CNIcon>
                </button>
              </div>
            );
          })}
...
Object.assign(window, { CNFavsScreen });
