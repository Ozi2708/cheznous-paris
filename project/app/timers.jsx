/* ── Minuteurs & conseils d'étape — partagés par les modes cuisine ──
   - cnStepDurations : détecte « 30 min », « 1 h », « 10 à 12 min » dans une étape
   - useCNTimers     : minuteurs persistants (localStorage, timestamps)
   - CNTimerChips    : chips « Lancer · 30 min » sous l'étape
   - CNActiveTimers  : pastilles compte à rebours, restent visibles d'étape en étape
   - cnAssignTips    : rattache astuces / conseils du livre à l'étape la plus proche */
const { useState: useStateT, useEffect: useEffectT, useMemo: useMemoT } = React;

const CN_TIMERS_KEY = 'cheznous_timers_v1';

function cnStepDurations(html) {
...
function useCNTimers() {
  const [timers, setTimers] = useStateT(cnLoadTimers);
...
/* Pastilles des minuteurs actifs — visibles sur toutes les étapes */
function CNActiveTimers({ timers, removeTimer, T }) {
      ...
      {timers.map(t => {
        ...
        return (
          <button key={t.id} onClick={() => removeTimer(t.id)} className={done ? 'cn-timer-done' : ''}
            ...
            <CNIcon name="clock" size={14} color={done ? '#FFFFFF' : T.accent}></CNIcon>
            ...
            <CNIcon name="x" size={11} color={done ? '#FFFFFF' : T.faint}></CNIcon>
          </button>
        );
      })}
    </div>
...
/* Chips de lancement pour l'étape courante */
function CNTimerChips({ html, label, timers, addTimer, T }) {
  const durations = useMemoT(() => cnStepDurations(html), [html]);
      ...
      {durations.map(mn => {
        ...
        return (
          <button key={mn} disabled={running} onClick={() => addTimer(label, mn)} style={{
            ...
            <CNIcon name="clock" size={15} color={T.accent}></CNIcon>
...
/* Rattache chaque astuce / conseil à l'étape qui lui ressemble le plus */
function cnAssignTips(recipe) {
  ...
  tips.forEach(tip => {
    ...
    let best = -1, score = 1; /* exige au moins 2 tokens communs */
...
/* Encart conseil sous une étape */
function CNStepTip({ tip, T }) {
  return (
    <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start', background: T.card, border: `1px solid ${T.cardBd}`, borderRadius: 11, padding: '11px 14px', marginTop: 14 }}>
      <CNIcon name="bulb" size={16} color={T.accent} style={{ flexShrink: 0, marginTop: 1 }}></CNIcon>
...
Object.assign(window, { useCNTimers, CNActiveTimers, CNTimerChips, CNStepTip, cnStepDurations, cnAssignTips });
