/* ── Fiche recette mobile — portions scalables + checklist ─────── */
const { useState: useStateR, useMemo: useMemoR } = React;

function CNTopBar({ onBack, label, color, onPrev, onNext, pos, fav, onFav }) {
  const arrowBtn = (dir, disabled, fn) => (
    <button onClick={fn} disabled={disabled} aria-label={dir > 0 ? 'Recette suivante' : 'Recette précédente'} style={{
      ...
      <CNIcon name={dir > 0 ? 'chevR' : 'back'} size={16} color="#3C3830"></CNIcon>
    </button>
  );
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 14px 8px' }}>
      <button onClick={onBack} aria-label="Retour" style={{
        ...
        <CNIcon name="back" size={18} color="#3C3830"></CNIcon>
      </button>
      ...
      {onFav && (
        <button onClick={onFav} aria-label={fav ? 'Retirer des favoris' : 'Ajouter aux favoris'} title={fav ? 'Retirer des favoris' : 'Ajouter aux favoris'} style={{
          ...
          <CNIcon name="heart" size={18} color="#B89268" fill={fav ? '#B89268' : 'none'}></CNIcon>
        </button>
      )}
      {(onPrev || onNext) && (
...
/* Stepper de portions — recalcule toutes les quantités */
...
/* Liste d'ingrédients interactive (visuel du système + cases à cocher) */
              ...
              return (
                <button key={ii} onClick={() => {
                  ...
                  <span style={{
                    ...
                    {done && <CNIcon name="check" size={13} color="#FFFFFF" strokeWidth={2.5}></CNIcon>}
                  </span>
...
function CNRecipeScreen({ recipe, onBack, onCook, portions, setPortions, week, onPlanWeek, onPrev, onNext, pos, bottomInset = 0, fav, onFav }) {
  const DS = window.ChezNousParisDesignSystem_b974d8 || {};
  const { NutritionBar, TipBox, InfoCard } = DS;
  const [checked, setChecked] = useStateR(() => new Set());
  const [planOpen, setPlanOpen] = useStateR(false);
  const [justPlanned, setJustPlanned] = useStateR(null);
  ...
  return (
    <div data-screen-label={'Fiche — ' + recipe.title} style={{ height: '100%', display: 'flex', flexDirection: 'column', background: '#FAFAF8', position: 'relative' }}>
      <div style={{ paddingTop: 56, flexShrink: 0 }}>
        <CNTopBar onBack={onBack} label={`${m.label} · Recette ${String(recipe.num).padStart(2, '0')}`} color={m.color} onPrev={onPrev} onNext={onNext} pos={pos} fav={fav} onFav={onFav}></CNTopBar>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', minHeight: 0, padding: `4px 20px ${130 + bottomInset}px` }}>
        ...
        {NutritionBar && <NutritionBar kcal={recipe.nutrition.kcal} lipides={recipe.nutrition.lipides} glucides={recipe.nutrition.glucides} proteines={recipe.nutrition.proteines}></NutritionBar>}
        ...
        <CNPortionStepper portions={portions} setPortions={setPortions} color={m.color}></CNPortionStepper>

        <div style={sectionLbl}>Ingrédients</div>
        <CNIngredientChecklist recipe={recipe} factor={factor} checked={checked} setChecked={setChecked}></CNIngredientChecklist>
        ...
        {recipe.tips.length > 0 && TipBox && (
          <div style={{ marginTop: 18 }}>
            {recipe.tips.map((t, i) => (
              <TipBox key={i} type={t.cls.includes('var') ? 'variante' : 'astuce'} title={t.title} text={t.text}></TipBox>
            ))}
          </div>
        )}

        <div style={sectionLbl}>Préparation</div>
        <CNStepsPreview steps={recipe.steps} color={m.color}></CNStepsPreview>

        {recipe.claudy.length > 0 && InfoCard && (
          <InfoCard advisor="Claudy" text={recipe.claudy[0]} subtitle={recipe.claudy[1] || undefined}></InfoCard>
        )}
            ...
            return (
              <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                <span style={{ width: 30, height: 30, borderRadius: '50%', background: fi.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <CNIcon name={fi.icon} size={15} color={fi.c}></CNIcon>
                </span>
                <div>
      ...
      {/* CTA sticky — lancer le mode cuisine + planifier */}
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: bottomInset, padding: '14px 20px 16px', background: 'linear-gradient(to top, #FAFAF8 65%, rgba(250,250,248,0))', display: 'flex', gap: 10, zIndex: 50 }}>
        <button onClick={onCook} style={{
          ...
          <CNIcon name="pot" size={20} color="#FFFFFF"></CNIcon>
        ...
        {onPlanWeek && (
          <button onClick={() => setPlanOpen(true)} aria-label="Ajouter à ma semaine" style={{
            ...
            <CNIcon name="cal" size={20} color="#506741"></CNIcon>
          </button>
        )}
      </div>

      {/* Sheet : choisir jour + créneau */}
      {onPlanWeek && (
...
Object.assign(window, { CNRecipeScreen, CNTopBar });
