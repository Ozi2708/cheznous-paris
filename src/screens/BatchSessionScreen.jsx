    ...
    <div data-screen-label="Session batch — 2h chrono" style={{ height: '100%', display: 'flex', flexDirection: 'column', background: T.bg, position: 'relative' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', minHeight: 0, padding: '22px 22px 16px', display: 'flex', flexDirection: 'column' }}>
                      ...
                      return (
                        <button key={ing.key} onClick={() => {
                          ...
                          <span style={{ width: 19, height: 19, borderRadius: 6, flexShrink: 0, border: `1.5px solid ${done ? color : T.ghostBd}`, background: done ? color : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            {done && <CNIcon name="check" size={12} color="#FAFAF8" strokeWidth={2.5} />}
                          </span>
        ...
        {it && (
          <div style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
              ...
              <CNTimerChips html={it.html} label={(it.recipe.title.length > 18 ? it.recipe.title.slice(0, 17) + '…' : it.recipe.title)} timers={timers} addTimer={addTimer} T={T} />
              {(cnAssignTips(it.recipe)[it.stepIdx] || []).map((tip, ti) => <CNStepTip key={ti} tip={tip} T={T} />)}
            </div>
            {stepIngs.length > 0 && (
        ...
        {phase >= n && (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', textAlign: 'center' }}>
            ...
            <div style={{ margin: '14px auto 24px' }}><CNBelgianDots size={6} gap={6} /></div>
      ...
      <div style={{ flexShrink: 0, display: 'flex', gap: 12, alignItems: 'center', padding: '12px 20px calc(env(safe-area-inset-bottom, 0px) + 16px)' }}>
        <button onClick={() => go(phase - 1)} disabled={phase <= -1} aria-label="Étape précédente" style={{
          ...
          <CNIcon name="back" size={22} color={T.ghostFg} />
        </button>
        {phase < n ? (
